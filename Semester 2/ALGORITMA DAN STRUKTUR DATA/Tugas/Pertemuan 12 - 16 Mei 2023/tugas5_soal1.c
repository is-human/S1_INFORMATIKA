...
void bagiRata(Queue *mahasiswa, Queue *bubur){
    ...
    int mahasiswa_fail_count = -1, bubur_fail_count = -1;
    //melakukan perulangan selama antrian mahasiwa tidak kosong
    while(mahasiswa->rear >= 0){
        //jika antrian mahasiswa masih tidak cocok maka putar antrian bubur
        if(mahasiswa_fail_count >= mahasiswa->rear){
            //memindahkan element terdepan ke akhir antrian bubur
            enqueue(bubur, bubur->data[0]);
            dequeue(bubur);
            bubur_fail_count++;
            //jika antrian bubur masih tidak cocok maka akhiri
            if(bubur_fail_count == bubur->rear) break;
        }
